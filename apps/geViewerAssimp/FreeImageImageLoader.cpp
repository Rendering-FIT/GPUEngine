...
FreeImageImage* FreeImageImageLoader::loadImage(const char* imageIdentifier)
{
   FreeImageImage *image = new FreeImageImage();
   fipImage *freeImage = new fipImage();

   if(!freeImage->load(imageIdentifier)) return NULL;
...
char* FreeImageImage::getBits()
{
   if(!_image) return NULL;
   return (char*)_image->accessPixels();
}
