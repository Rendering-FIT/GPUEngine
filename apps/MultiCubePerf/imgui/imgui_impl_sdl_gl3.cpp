...
void ImGui_ImplSdlGL3_RenderDrawLists(ImDrawData* draw_data)
{
    ...
    // Backup GL state
    GLint last_program; gl->glGetIntegerv(GL_CURRENT_PROGRAM, &last_program);
    GLint last_texture; gl->glGetIntegerv(GL_TEXTURE_BINDING_2D, &last_texture);
    GLint last_active_texture; gl->glGetIntegerv(GL_ACTIVE_TEXTURE, &last_active_texture);
    GLint last_array_buffer; gl->glGetIntegerv(GL_ARRAY_BUFFER_BINDING, &last_array_buffer);
    GLint last_element_array_buffer; gl->glGetIntegerv(GL_ELEMENT_ARRAY_BUFFER_BINDING, &last_element_array_buffer);
    GLint last_vertex_array; gl->glGetIntegerv(GL_VERTEX_ARRAY_BINDING, &last_vertex_array);
    GLint last_blend_src; gl->glGetIntegerv(GL_BLEND_SRC, &last_blend_src);
    GLint last_blend_dst; gl->glGetIntegerv(GL_BLEND_DST, &last_blend_dst);
    GLint last_blend_equation_rgb; gl->glGetIntegerv(GL_BLEND_EQUATION_RGB, &last_blend_equation_rgb);
    GLint last_blend_equation_alpha; gl->glGetIntegerv(GL_BLEND_EQUATION_ALPHA, &last_blend_equation_alpha);
    GLint last_viewport[4]; gl->glGetIntegerv(GL_VIEWPORT, last_viewport);
    GLboolean last_enable_blend = gl->glIsEnabled(GL_BLEND);
    GLboolean last_enable_cull_face = gl->glIsEnabled(GL_CULL_FACE);
    GLboolean last_enable_depth_test = gl->glIsEnabled(GL_DEPTH_TEST);
    GLboolean last_enable_scissor_test = gl->glIsEnabled(GL_SCISSOR_TEST);
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (const ImDrawCmd* pcmd = cmd_list->CmdBuffer.begin(); pcmd != cmd_list->CmdBuffer.end(); pcmd++)
        {
            ...
            else
            {
                ...
                gl->glScissor((int)pcmd->ClipRect.x, (int)(float(fb_height) - pcmd->ClipRect.w), (int)(pcmd->ClipRect.z - pcmd->ClipRect.x), (int)(pcmd->ClipRect.w - pcmd->ClipRect.y));
...
bool ImGui_ImplSdlGL3_CreateDeviceObjects()
{
    ...
    gl->glShaderSource(g_VertHandle, 1, &vertex_shader, 0);
    gl->glShaderSource(g_FragHandle, 1, &fragment_shader, 0);
    ...
    gl->glGenVertexArrays(1, &g_VaoHandle);
    gl->glBindVertexArray(g_VaoHandle);
    gl->glBindBuffer(GL_ARRAY_BUFFER, g_VboHandle);
...
#define OFFSETOF(TYPE, ELEMENT) ((size_t)&(((TYPE *)0)->ELEMENT))
    gl->glVertexAttribPointer(g_AttribLocationPosition, 2, GL_FLOAT, GL_FALSE, sizeof(ImDrawVert), (GLvoid*)OFFSETOF(ImDrawVert, pos));
    gl->glVertexAttribPointer(g_AttribLocationUV, 2, GL_FLOAT, GL_FALSE, sizeof(ImDrawVert), (GLvoid*)OFFSETOF(ImDrawVert, uv));
    gl->glVertexAttribPointer(g_AttribLocationColor, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(ImDrawVert), (GLvoid*)OFFSETOF(ImDrawVert, col));
#undef OFFSETOF
...
{
    if (g_VaoHandle) gl->glDeleteVertexArrays(1, &g_VaoHandle);
...
void ImGui_ImplSdlGL3_NewFrame(SDL_Window* window)
{
    ...
    io.DisplayFramebufferScale = ImVec2(w > 0 ? ((float)display_w / float(w)) : 0, h > 0 ? ((float)display_h / float(h)) : 0);
